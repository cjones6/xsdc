"""
Train a CKN-GM on the CIFAR-10 dataset
"""
...
parser = argparse.ArgumentParser(description='CKN-GM training on the CIFAR-10 dataset')
...
parser.add_argument('--save_path', default='../../results/cifar10_ckn-gm/temp', type=str,
...
train_loader, _, train_labeled_loader, train_unlabeled_loader, valid_loader, train_valid_loader, test_loader = \
    cifar10.get_dataloaders(batch_size=args.batch_size, batch_size_labeled=args.batch_size_labeled,
...
save_file = save_dir + str(args.num_labeled) + '_' + str(-1) + '_' + str(args.num_filters) + '_' + str(bw) + \
            ...
            '-'.join(str(args.labeling_method).split(' ')) + '_' + str(args.lam) + '_' + str(max_frac_points_class) + \
            '_' + str(args.num_clusters) + '_' + str(args.update_clusters_every) + \
...
params = opt_structures.Params(nclasses=nclasses, min_frac_points_class=min_frac_points_class,
                               max_frac_points_class=max_frac_points_class, ckn=ckn, project=True, lam=args.lam,
