...
def ulr_square_loss_y(features, y_one_hot, lam, lam_pix, penalize=True):
    """
    ...
    :param lam_pix: Regularization parameter for the centered features
    :param penalize: Whether to subtract a penalty on the centered features
    ...
    if penalize:
        obj = 1 / n * (torch.sum((label_utils.centering(y_one_hot)) ** 2) - torch.trace(xpiy.t().mm(wlast))) - \
              lam_pix / n * torch.sum(label_utils.centering(features)**2)
    else:
        obj = 1 / n * (torch.sum((label_utils.centering(y_one_hot)) ** 2) - torch.trace(xpiy.t().mm(wlast)))
    w_last = wlast.detach()
...
def ulr_square_loss_m(features, M, lam, lam_pix, penalize=True):
    """
    ...
    A = lam*pi.mm(A)
    if penalize:
        obj = torch.sum(M * A) - lam_pix / n * torch.sum(label_utils.centering(features)**2)
    else:
        obj = torch.sum(M * A)
